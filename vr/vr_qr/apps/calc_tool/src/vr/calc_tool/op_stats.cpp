...
run (std::string const & cfg_name, fs::path const & in_root, util::date_t const & date, std::string const & tz, fs::path const & out_root)
{
    ...
    app.start ();
    {
        ...
        using book_type         = limit_order_book<price_si_t, oid_t, user_data<stats_metadata>, level<_qty_, _order_count_>>;
...
//        LOG_info << "[glimpse DONE]";
        ...
        // continue by consuming mcast data:
        {
            ...
            v.get<stats> ().emit (std::cerr); // dump stats
        }
        LOG_info << "[itch DONE]";
    }
    app.stop ();
...
int32_t
op_stats (string_vector const & av)
{
    std::string cfg { };
    fs::path in_root { };
    fs::path out_root { };
    ...
    bpopt::options_description opts { "usage: " + sys::proc_name () + " stats [options] file" };
    opts.add_options ()
    ...
    try
    {
        ...
        run (cfg, in_root, date, tz, out_root);
    }
