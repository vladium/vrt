...
VR_APP_MAIN ()
{
    ...
    opts.add_options ()
        ("op",          bpopt::value<std::string> ()->value_name ("<op>")->required (),  "operation (itch|ouch)")
    ...
    try
    {
        ...
        {
            case "itch"_hash:   rc = op_itch_session (op_args); break;
            case "ouch"_hash:   rc = op_ouch_session (op_args); break;
